...
export default class PriorityController {
  ...
  static async createPriority(req, res) {
    ...
    try {
      ...
      const checkExistenceQuery =
        "SELECT ptid FROM priority WHERE (code = $1 OR priority = $2) And active = 'Y'";
      ...
      const validationErrors = ValidateData({ code, priority });
      if (validationErrors.length > 0) {
        return SendError400(res, validationErrors);
      }

      const sqlQuery =
        "INSERT INTO priority (code, priority, createdate, createby, active) VALUES ($1, $2, NOW(), $3, 'Y') RETURNING ptid";
  ...
  static async updatePriority(req, res) {
    ...
    try {
      ...
      await client.query("BEGIN"); // Start transaction
      const checkExistenceQuery =
        "SELECT ptid FROM priority WHERE (code = $1 AND priority = $2) And active = 'Y'";
      const existenceResult = await client.query(checkExistenceQuery, [
        code.trim(),
        priority.trim(),
      ]);
      ...
      //this is update data
      const sqlQuery =
        "UPDATE priority SET code = $1, priority = $2 WHERE ptid = $3";
  ...
  static async deletePriority(req, res) {
    ...
    try {
      ...
      await client.query("BEGIN"); // Start transaction
      const checkExistenceQuery =
        "SELECT ptid FROM priority WHERE ptid = $1 And active = 'Y'";
      ...
      //this is update data
      const sqlQuery =
        "UPDATE priority SET active = 'N' WHERE ptid = $1 RETURNING ptid";
